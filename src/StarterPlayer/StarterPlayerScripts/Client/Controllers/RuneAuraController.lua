...
local function makeOrb(folder: Folder, style, index: number)
	...
	orb.Transparency = style.transparency
	orb.Anchored = true
	...
	activeAura.conn = RunService.RenderStepped:Connect(function()
		...
		for index, orb in ipairs(activeAura.orbs) do
			if orb and orb.Parent then
				...
				local yOffset = math.sin(elapsed * 2 + index) * style.bobAmplitude + 2.2
