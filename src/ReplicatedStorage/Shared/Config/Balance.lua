...
--========================================
-- XP 획득량 (Phase 6 - 밸런스 복구)
--========================================
Balance.XP_CREATURE_KILL = 100           -- 크리처 처치 (200000 -> 100)
Balance.XP_CRAFT_ITEM = 20              -- 아이템 제작 (50000 -> 20)
Balance.XP_CAPTURE_PAL = 200            -- 팰 포획 성공 (500000 -> 200)
Balance.XP_HARVEST_RESOURCE = 10        -- 자원 채집 (1000000 -> 10)
Balance.XP_BUILD = 15                  -- 구조물 배치
Balance.XP_COOK_ITEM = 25              -- 요리 XP
Balance.XP_PROCESS_ITEM = 20           -- 가공/제련 XP
...
-- 플레이어 스탯 보너스 (Phase 6)
--========================================
Balance.HP_PER_POINT = 4               -- 포인트당 체력 증가 (10 -> 4)
Balance.STAMINA_PER_POINT = 2          -- 포인트당 스태미너 증가 (10 -> 2)
...
Balance.ATTACK_PER_POINT = 0.02        -- 포인트당 공격력 증가 (5% -> 2%)
...
-- 스프린트 (빠르게 달리기)
Balance.SPRINT_SPEED_MULT = 1.50       -- 스프린트 속도 배율 (1.25 -> 1.50)
...
Balance.DODGE_COOLDOWN = 1.2           -- 구르기 쿨타임 (3.0 -> 1.2)
...
--========================================
-- 텔레그래프 공격 시스템 (크리처 공격 패턴)
--========================================
-- 패턴별 기본 선행(windup) 시간 (크리처 개별 설정이 우선)
Balance.TELEGRAPH_WINDUP_CONE = 0.35       -- 부채꼴: 짧음 (즉발에 가까움)
Balance.TELEGRAPH_WINDUP_CIRCLE = 1.2      -- 원형 범위: 길음
Balance.TELEGRAPH_WINDUP_CHARGE = 1.5      -- 직선 돌진: 길음
Balance.TELEGRAPH_WINDUP_PROJECTILE = 0.4  -- 원거리: 짧음
-- 범위 표시 시각 효과 (비활성 — 크리처 Highlight 발광으로 대체)
-- Balance.TELEGRAPH_INDICATOR_COLOR = Color3.fromRGB(255, 240, 130)
-- Balance.TELEGRAPH_INDICATOR_OPACITY = 0.35
-- Balance.TELEGRAPH_INDICATOR_HEIGHT = 0.15
-- Balance.TELEGRAPH_INDICATOR_OVERSIZE = 1.15
-- Balance.TELEGRAPH_FLASH_SPEED = 3.0
